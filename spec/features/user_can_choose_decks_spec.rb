...
RSpec.describe "user can choose a deck", type: :feature do
  VCR.use_cassette "logged_in#user_chooses_a_deck" do
    scenario "user can choose a deck upon logging in" do
      ...
      click_link "#{deck_question_two.deck.name}"

      expect(page).to have_current_path flashcard_user_path(user, deck_question_two.deck.id)

      # expect page to have cards specific to deck
    end
  end
end
